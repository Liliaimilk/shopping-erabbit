<template>
  <HomePanel title="最新专题">
    ...
    <div class="special-list" ref="homeSpecial">
      <div class="special-item" v-for="i in goods" :key="i">
        <RouterLink to="/">
          <img :src="i.cover" alt />
          <div class="meta">
            <p class="title">
        ...
        <div class="foot">
          <span class="like"
            ><i class="iconfont icon-hart1"></i>{{ i.collectNum }}</span
          >
          <span class="view"
            ><i class="iconfont icon-see"></i>{{ i.viewNum }}</span
          >
          <span class="reply"
            ><i class="iconfont icon-message"></i>{{ i.replyNum }}</span
          >
        </div>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    // });
    const { target, goods } = useLazyData(findSpecial);
    return { goods, homeSpecial: target };
  },
};
</script>
